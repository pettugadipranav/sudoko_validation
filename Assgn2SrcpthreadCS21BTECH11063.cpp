...
int main()
{
    ...
    pthread_t threadIDs, *apt;
    apt = (pthread_t *)calloc(sizeof(pthread_t), k);

    auto start_time = high_resolution_clock::now(); // starting point

    for (int i = 0; i < k; i++)
    {
        int *arg = (int *)calloc(sizeof(*arg), 1);
        *arg = i;
        pthread_create(&threadIDs, 0, func1, arg);
        apt[i] = threadIDs;
    }
    ...
    FILE *fptr = fopen("Outmain1.txt", "w"); // opening the file to write the output
    for (int i = 0; i < k; i++)
    {
        int q1 = q;
        if (i < r)
